...
% Modify 130819 JH
loc = strfind(A,'Latitude=');
H.latitude = sscanf(A(loc(1)+length(['Latitude=']):loc(1)+20),'%f\n');

loc = strfind(A,'Longitude=');
searchCR = strfind(A(loc(1):end),[char(8)]);
H.longitude = sscanf(A(loc(1)+length(['Longitude=']):loc(1)+20),'%f\n');
